  ...
  <v-app>
    <v-container>
      ...
      <div v-for="(c, id) in colors" :key="id">
        <v-btn :color="c.color" block>{{ c.color }}</v-btn>
      </div>
    </v-container>
  </v-app>
</template>
  ...
  methods: {
    async ct_func() {
      ...
      for (let i = 0; i < this.colors.length; ++i) {
        ...
        this.colors[
          i
        ].color = `rgb(${this.colors[i].value[0]},${this.colors[i].value[1]},${this.colors[i].value[2]})`
      }
      this.primary = `rgb(${this.primary[0]},${this.primary[1]},${this.primary[2]})`
    },
    loadImage(url, elem) {
